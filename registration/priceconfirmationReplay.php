...
function pickingConfirmationReply($chat_id)
{
    ...
    while ($ro = mysqli_fetch_array($companyOwnerQuery)) {
        ...
        $admin_id = $ro['telegram_id'];
        file_get_contents($botAPI . "/sendmessage?chat_id=" . $admin_id . "&text=Dear Owner, There is a new picking rate request you need to approve.");
    }
    $info = "";
    $info .= "Your rate request have been sent.";
    file_get_contents($botAPI . "/sendmessage?chat_id=" . $chat_id . "&text=" . $info . "&parse_mode=html");
    superAdminMenu($chat_id);
}
