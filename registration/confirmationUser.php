...
function savedata($chat_id)
{
    ...
    file_get_contents($botAPI . "/sendmessage?chat_id=" . $chat_id . "&text=Hello" . "%0A" . "You have been registered as " . $roleBold . " for $companyName company. In order to access the please click the following link. @MytrstingBot &parse_mode=html");
...
function savebuyerdata($chat_id)
{
    ...
    $selectownerscompany = "SELECT * FROM company_users where company_telegram_id='$chat_id'";
    $selectownerscompanyQuery = mysqli_query($con, $selectownerscompany);
    while ($co = mysqli_fetch_array($selectownerscompanyQuery)) {
        $companyName = $co['company_name'];
    }
    ...
    $roleBold = "<b>$role</b>";
    file_get_contents($botAPI . "/sendmessage?chat_id=" . $chat_id . "&text=" . $conc . "Please forward the message below to this username @$telegram_username.");
    file_get_contents($botAPI . "/sendmessage?chat_id=" . $chat_id . "&text=Hello" . "%0A" . "You have been registered as an " . $roleBold . " for $companyName company. In order to access the please click the following link. @MytrstingBot &parse_mode=html");
    $deletadmindatafromtemp = "DELETE FROM company_users_temp WHERE telegram_username='$telegram_username'";
    mysqli_query($con, $deletadmindatafromtemp);
}
