...
function report($chat_id)
{
    ...
    if ($TransactionRow > 0) {
        while ($ro = mysqli_fetch_array($listDailyTransactionQuery)) {
            ...
            $marksHTML .= "<b>Price :-</b> " . strtolower($price) . "%0A";
            $hel .= $marksHTML;
            $viewSeller = "v_";
...
function previewSeller($full_name, $chat_id)
{
    ...
    if ($listSellerQueryrow > 0) {
        ...
        $marksHTML .= "<b>Registerd by:-</b>" . strtolower($adminFirstname . " " . $adminLastname) . "%0A";
        $marksHTML .= "<b>Date of registration:-</b>" . strtolower($date_registered) . "%0A";
        $hel .= $marksHTML;
